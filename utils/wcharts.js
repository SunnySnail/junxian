var Charts = (function() {
        ...
        series.forEach(function(item, itemIndex) {
            ...
            if(option.drawPoint.length > 0) {
                option.drawPoint.forEach(function(point) {
                    let x = xPoints[point];
                    let y = height-offset-((option.yArisData[point].value-minYArisData)*yDotLen).toFixed(4);
                    let pointData = {
                        x: x,
     ...
     * @return {[]}
     */
    function drawLine(option, offset) {
        setLineStyle(option);
        context.beginPath();
        context.moveTo(0, height-offset-(option.yArisData[0].value-minYArisData) * yDotLen);
    ...
    function drawTipBox(x, y, pointData, tipsBoxes) {
        ...
        tipsBoxes.forEach(function(item, index) {
            ...
            let _x = x - 10 - item.width - lastWidth;
            let _y = y - 10;
            ...
            context.fillText(item.tipsContent, _x+8, _y+13);
        })
    }
